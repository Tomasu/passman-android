package es.wolfi.app.passman.widget;
 ...
 */
public
class LoadingButton extends AppCompatButton implements View.OnTouchListener
{
	...
	@SuppressLint ("ClickableViewAccessibility")
	public
	LoadingButton ( Context context, AttributeSet attrs )
	{
		super( context, attrs );
		init();
	...
	@SuppressLint ("ClickableViewAccessibility")
	public
	LoadingButton ( Context context, AttributeSet attrs, int defStyleAttr )
	{
	...
	void parseAttrs ( Context context, AttributeSet attrs )
	{
		...
		for ( int i = 0; i < typedArray.getIndexCount(); i++ )
		{
			...
			{
				mButtonColor = typedArray.getColor( attr, getResources().getColor(
						R.color.unpressed_color ) );
			}
			...
			{
				mPaddingProgress = mCornerRadius = typedArray.getDimensionPixelSize( attr,
																											R.dimen.fbutton_default_progress_margin );
			}
			else if ( attr == R.styleable.LoadingButton_lb_loaderWidth )
			{
				mStrokeWidth = mCornerRadius = typedArray.getDimensionPixelSize( attr,
																									  R.dimen.fbutton_default_progress_width );
	...
	private
	LayerDrawable createDrawable ( int radius, int topColor, int bottomColor )
	{

		float[] outerRadius = new float[] {
	...
	boolean onTouch ( View view, MotionEvent motionEvent )
	{
		switch ( motionEvent.getAction() )
		{
			...
			case MotionEvent.ACTION_MOVE:
				...
				if ( !r.contains( (int) motionEvent.getX(),
										(int) motionEvent.getY() + 3 * mShadowHeight ) && !r.contains( (int) motionEvent.getX(), (int) motionEvent.getY() - 3 * mShadowHeight ) )
				{
					updateBackground( unpressedDrawable );
					this.setPadding( mPaddingLeft, mPaddingTop + mShadowHeight, mPaddingRight,
										  mPaddingBottom + mShadowHeight );
	...
	private
	void drawIndeterminateProgress ( Canvas canvas )
	{
